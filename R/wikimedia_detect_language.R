...
wikimedia_detect_language <- function(text) {
  ...
  response <- safe_http(httr::POST(url, body = json_body, httr::add_headers(headers)), "Wikimedia Language ID API")
  if (is.null(response)) return(invisible(NULL))

  # Check if the request was successful
  if (httr::status_code(response) != 200) {
    message("Wikimedia Language ID API request failed with status ", httr::status_code(response))
